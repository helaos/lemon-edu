      ...
      <div>
        ...
        <section class="">
          <section class="question-list lh-bj-list pr">
            <ul class="pr10">
              <li v-for="(comment,index) in data.rows" v-bind:key="index">
                <aside class="noter-pic">
                ...
                <div class="of mt5">
                  <span class="fr">
                    <font class="fsize12 c-999 ml5">{{comment.createTime}}</font>
                  </span>
                </div>
              </li>
            </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      createOrders() {
        order.createOrders(this.courseId)
          .then(response => {
            // 获取返回的订单号
            // 生成订单之后，跳转到订单显示页面
            this.$router.push({
              path: '/orders/' + response.data.data.no
            });
          })
      }
    }
